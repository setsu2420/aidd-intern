...
async def test_binder_design_latent_y_campaign_modalities_and_steering():
    ...
    assert (
        plan1["hitl_steering_options"][0]["option_id"] == "publication_direct_extract"
    )
    ...
    text2, ok2 = await binder_design_handler(
        {
            ...
            "requirements": {"species_cross_reactivity": ["human", "cynomolgus"]},
        }
    )
    assert ok2 is True
    ...
    assert any(
        opt["option_id"] == "cross_species_dual_optimization"
        for opt in plan2["hitl_steering_options"]
    )
