...
- `configs/`: CLI and frontend agent defaults. Update both
  `cli_agent_config.json` and `frontend_agent_config.json` when changing shared
  MCP servers, model defaults, trace behavior, or domain-pack defaults.
...
- Keep local secrets in root `.env` or shell exports. Common variables:
  ...
  `AIDD_INTERN_DEFAULT_MODEL_ID`, `LOCAL_LLM_BASE_URL`, and
  `AIDD_INTERN_PROTEINMCP_HOME`.
...
- The default CLI/web configs register Hugging Face MCP and local ProteinMCP
  stdio launchers. Use `scripts/setup-proteinmcp-local.sh all` for the heavy
  one-time setup.
