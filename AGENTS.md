...
- Backend: from `backend/`, run `uv run python -m uvicorn main:app --host ::1 --port 7860`.
...
- Production defaults to the Bedrock Claude model. For local development with a personal Anthropic key, set `ANTHROPIC_API_KEY` and `AIDD_INTERN_CLAUDE_MODEL_ID=anthropic/claude-opus-4-6` before starting the backend. Other models are selected through the app's model switcher.
...
- The Space remote is `space` and points to `https://huggingface.co/spaces/smolagents/aidd-intern`.
