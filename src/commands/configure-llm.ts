...
export function runConfigureLlm(provider?: string): boolean {
  const normalized = provider?.trim().toLowerCase();
  const guide = normalized ? providerGuides[normalized] : undefined;
...
function printGuide(guide: ProviderGuide): void {
