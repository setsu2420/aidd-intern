...
function createProgram() {
  ...
  program2.command("smoke").description("Health check, session lifecycle, and model config").action(async () => {
    const env = resolveEnv(program2.opts());
    await runCommand(env, async () => {
      const { runSmoke } = await import("./smoke-GIFRYS4M.js");
      return runSmoke(env);
    });
  });
  ...
  program2.command("configure-llm").description("Print provider-specific LLM environment setup steps").argument("[provider]", "openrouter, openai, anthropic, siliconflow, or local").action(async (provider) => {
    const env = resolveEnv(program2.opts());
    await runCommand(env, async () => {
      const { runConfigureLlm } = await import("./configure-llm-4JDLNW6F.js");
      return runConfigureLlm(provider);
    });
  });
  program2.command("integration").description("Chat flow, SSE streaming, and tool execution").action(async () => {
    const env = resolveEnv(program2.opts());
    await runCommand(env, async () => {
      const { runIntegration } = await import("./integration-MZWVV35E.js");
      return runIntegration(env);
    });
  });
  program2.command("eval").description("Agent capability benchmarks and trace quality analysis").option("--judge", "Enable LLM-as-judge scoring").option("--fixtures <path>", "Path to the evaluation fixture bundle").option("--limit <number>", "Limit the number of evaluation cases", (value) => {
    ...
    const env = resolveEnv(program2.opts());
    await runCommand(env, async () => {
      const { runEval } = await import("./eval-R2GUTBHB.js");
      return runEval(env, {
