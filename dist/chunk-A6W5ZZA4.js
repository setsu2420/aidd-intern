...
// src/client/types.ts
var ApiRootSchema = objectType({
  name: stringType(),
  version: stringType()
});
var HealthResponseSchema = objectType({
  status: stringType(),
  active_sessions: numberType(),
  max_sessions: numberType()
});
var LLMHealthResponseSchema = objectType({
  status: stringType(),
  model: stringType(),
  error: stringType().optional(),
  error_type: stringType().optional()
});
var SessionResponseSchema = objectType({
  session_id: stringType(),
  ready: booleanType(),
  model: stringType()
});
var SessionInfoSchema = objectType({
  session_id: stringType(),
  is_active: booleanType(),
  model: stringType(),
  turn_count: numberType(),
  runtime_state: stringType().optional(),
  title: stringType().nullable().optional(),
  pending_tools: arrayType(recordType(unknownType())).nullable().optional(),
  auto_approval: recordType(unknownType()).optional()
});
var SessionInfoListSchema = arrayType(SessionInfoSchema);
var ModelEntrySchema = objectType({
  id: stringType(),
  label: stringType(),
  provider: stringType(),
  tier: stringType(),
...
var ModelConfigSchema = objectType({
  current: stringType(),
  available: arrayType(ModelEntrySchema)
});
...
//# sourceMappingURL=chunk-A6W5ZZA4.js.map