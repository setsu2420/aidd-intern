...
STRUCTURE_CACHE_DIR = Path(__file__).parent.parent.parent / "data" / "structures"


@router.get("/v3/structure/pdb/{pdb_id}")
async def get_pdb_structure(
    pdb_id: str,
    file_format: str = Query("pdb", pattern="^(pdb|cif|bcif)$"),
    user: dict = Depends(get_current_user),
):
    """Fetch and cache RCSB PDB structure files.

    Automatically streams from RCSB PDB and atomic-caches locally to
    data/structures to eliminate load latency for subsequent hits.
    """
    pdb_id = pdb_id.strip().upper()
    if not pdb_id.isalnum() or len(pdb_id) != 4:
        raise HTTPException(
            status_code=400,
            detail="Invalid PDB ID. Must be 4 alphanumeric characters.",
        )

    STRUCTURE_CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache_file = STRUCTURE_CACHE_DIR / f"{pdb_id}.{file_format}"
    ...
    temp_cache_file = STRUCTURE_CACHE_DIR / f"{pdb_id}.{file_format}.tmp"
    ...
    async def stream_and_cache_generator():
        try:
                ...
                async with client.stream("GET", download_url) as response:
                    if response.status_code != 200:
                        raise HTTPException(
                            ...
                            detail=f"Failed to fetch PDB {pdb_id} from RCSB PDB",
                        )
                    ...
                    # Atomic swap to avoid partial read corruption
            ...
            raise HTTPException(
                status_code=500, detail=f"Error downloading structure: {e}"
            )
...
@router.get("/v3/structure/local")
async def get_local_structure(
    filepath: str = Query(
        ...,
        description="Absolute path or relative path to local PDB/CIF file in workspace",
    ),
    user: dict = Depends(get_current_user),
):
    """Safely stream PDB/CIF structure files generated by AI agent workflow.

    Enforces workspace directory confinement and file extensions for strict
    sandboxing security.
    """
    path = Path(filepath).resolve()
    workspace_root = Path(__file__).parent.parent.parent.resolve()

    # Sandboxing directory traversal checks
    if not str(path).startswith(str(workspace_root)):
        raise HTTPException(
            status_code=403,
            detail="Access forbidden: file lies outside workspace sandbox.",
        )

    if path.suffix.lower() not in {".pdb", ".cif", ".bcif"}:
        raise HTTPException(
            status_code=400,
            detail="Forbidden file type. Only .pdb, .cif and .bcif structure formats allowed.",
        )
