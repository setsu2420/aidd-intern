...
- Node.js 22+ と npm。Node package 用です。Git は source checkout または
  frontend 開発時だけ必要です。
- Python 3.11+、`uv`、Git。source checkout または backend を触る場合だけ必要です。
- PXDesign、BindCraft などをローカル実行する場合のみ Conda/Mamba と GPU

### Node package のインストール
...
GitHub archive tarball を直接インストールし、committed prebuilt `dist/` package
files を使います。この経路は local Git や GitHub SSH key に依存しません。
...
GitHub から global install した Node package は次のコマンドで更新します:
...
`aidd-intern update` は global install された Node harness package だけを更新します。
source checkout は編集せず、Python の `uv tool install -e .` runtime も更新しません。
現在の `aidd-intern` command が Python CLI を指している場合は、下の
